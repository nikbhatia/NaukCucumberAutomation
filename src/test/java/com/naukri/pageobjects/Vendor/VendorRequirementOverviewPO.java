...
public class VendorRequirementOverviewPO extends BaseAutomation {
	...
	public  By messageSuccess=By.xpath("//div[@class='psuccess']//following::span[contains(text(),'${message}')]");
	...
	public By lstOfVendors=By.xpath("//span[@class='vendorName']");
	public By eleMoreActionsDD=By.xpath("//a[contains(text(),'${requirmentname}')]//following::li[11]//a//small");
	public By eleAssignVendor=By.xpath("//a[contains(text(),'Assign Vendor')][1]");
	public By eleSources= By.xpath("//a[contains(text(),'${requirement}')]//following::a[contains(text(),'Sources')]");
...
//		pageRefresh();
	...
	public boolean verifySelectedVendorName(List<String>listOfVendorsToBeSelected){
		List<String>listChipOfVendorName=new ArrayList<>();
		for(int i=0;i<listChipOfVendor.size();i++)
			listChipOfVendorName.add(listChipOfVendor.get(i).getText());
		return(listOfVendorsToBeSelected.containsAll(listChipOfVendorName));
	}
	/**
	...
	/**
	 * This method is use to verify the assigned vendor are visible on the requirement overview page
	 * @return true or false
	 * @author avani
	 */
	public boolean verifyVendorsVisibleOnRequirementOverviewPage(List<String> listOfVendors){
		List<WebElement> listOfVendorsAssigned=getElements(lstOfVendors);
		for(int i=0;i<listOfVendors.size();i++)
			return(listOfVendorsAssigned.get(i).getText().contains(listOfVendors.get(i)));
		return  false;
	}
	/**
	 * This method is use to click on assign vendor option available on requirement listing page below the requirement
	 * appears clicking on More Actions
	 * @author avani
	 */
	public void selectAssignVendorFromAllRequirementsPage(){
		wait.waitForPageToLoadCompletely();
		actionBuilderUtil.hoverByActionAlternate(parameterizedLocator(eleMoreActionsDD, AddASingleProfilePagePO.requirementTitle));
		click(eleAssignVendor);
	}
	/**
	 * This method is use to click on source link on requirement listing page
	 * @author avani
	 */

	public void clickOnSourcesOnAllRequirementsPage(){
		wait.waitForPageToLoadCompletely();
		click(parameterizedLocator(eleSources,AddASingleProfilePagePO.requirementTitle));

	}
	
	/**
	 * @author gupta.mansi
	 * This method determines whether textarea for message vendor is displayed or not
	 * @return true if message box is present
	 */
	public boolean verifyPresenceOfMessageBox()
	{
